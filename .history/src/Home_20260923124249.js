...
// job | event | grant | training | public
const CATEGORY_META = {
...
const INFO_CONTENT = {
  ...
  privacy: {
    ...
    paragraphs: [
      'Browsing, searching, filtering, and bookmarking happen on your device. Bookmarks and display preferences are stored in your browser local storage and never sent to our servers.',
...
const BOOKMARK_IDLE =
  'p-2.5 rounded-full bg-surface-container-low hover:bg-secondary-container text-on-surface hover:text-on-secondary-fixed transition-colors';
const BOOKMARK_ACTIVE =
  'p-2.5 rounded-full bg-secondary-container text-on-secondary-fixed transition-colors';
    ...
    <DialogShell label={`Details: ${title}`} onClose={onClose} maxWidth="max-w-3xl">
      ...
      <div className="flex flex-col gap-5 px-space-lg py-space-md">
        <div className="flex flex-wrap gap-2">
          ...
          {deadline && <span className="rounded-full bg-surface-container-low px-3 py-1 font-label-sm text-label-sm font-semibold text-on-surface">Apply by {deadline}</span>}
...
function Home() {
  ...
  const [bookmarked, setBookmarked] = useState(() => {
    const stored = readStored(BOOKMARKS_STORAGE_KEY, []);
  ...
  // Persist display preferences and bookmarks on this device.
  useEffect(() => {
    writeStored(A11Y_STORAGE_KEY, { highContrast, largeText, dyslexiaFont });
  }, [highContrast, largeText, dyslexiaFont]);

  useEffect(() => {
    writeStored(BOOKMARKS_STORAGE_KEY, bookmarked);
  }, [bookmarked]);
  ...
  const handleDeleted = useCallback(
    (opp) => {
      ...
      setBookmarked((prev) => prev.filter((id) => id !== opp.id));
  ...
  const applyFilters = () => {
    ...
    pushToast(`Filters applied — ${count} ${count === 1 ? 'result' : 'results'}.`, 'info');
  };
  ...
  return (
    ...
    <div className={`bg-surface font-body-md text-body-md text-on-surface antialiased${highContrast ? ' high-contrast-mode' : ''}${dyslexiaFont ? ' dyslexia-font' : ''}`}>
      ...
      <h1 className="font-headline-xl text-headline-xl text-on-surface tracking-tight font-extrabold">
                  Discover Accessible &amp; Inclusive Opportunities
                </h1>
      <p className="font-body-xl text-body-xl text-on-surface-variant max-w-xl">
                  Find careers, internships, and fellowships built with accommodations from day one. Designed for dignity, full participation, and career progression.
                </p>
            ...
            <article key={opp.id} className="bg-surface-container-lowest rounded-2xl p-space-lg shadow-sm hover:shadow-md transition-shadow flex flex-col justify-between gap-space-md relative overflow-hidden group">
              <div className="flex flex-col gap-space-sm">
                ...
                <div className="flex items-start justify-between gap-3">
                  <div className="flex items-center gap-3">
                    {logoUrl ? (
                      <img src={logoUrl} alt={`${orgName} logo`} className="w-12 h-12 rounded-2xl object-cover bg-surface-container-low border border-surface-container" loading="lazy" />
                    ) : (
                      <div className={`w-12 h-12 rounded-2xl flex items-center justify-center font-headline-sm font-bold ${avatarStyle}`}>
                        {getInitials(orgName)}
                      </div>
                    )}
                    <div className="flex flex-col">
                  ...
                  <div className="flex shrink-0 items-center gap-1" data-menu-root>
                    ...
                    <div className="relative">
                      <button aria-label={`More actions for ${title}`} aria-haspopup="menu" aria-expanded={openMenuId === opp.id} className="rounded-full p-2.5 text-on-surface transition-colors hover:bg-secondary-container hover:text-on-secondary-fixed" onClick={() => setOpenMenuId((current) => (current === opp.id ? null : opp.id))} type="button"><span className="material-symbols-outlined text-[20px]">more_horiz</span></button>
                      {openMenuId === opp.id ? (
                        <div role="menu" aria-label={`Actions for ${title}`} className="absolute right-0 top-11 z-30 w-56 rounded-2xl bg-surface-container-lowest p-1.5 shadow-xl ring-1 ring-outline-variant">
                          <button role="menuitem" className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left font-body-sm text-body-sm text-on-surface hover:bg-surface-container" onClick={() => { setOpenMenuId(null); setDetailsOpp(opp); }} type="button"><span className="material-symbols-outlined text-[18px]" aria-hidden="true">visibility</span>View details</button>
                          <button role="menuitem" className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left font-body-sm text-body-sm text-on-surface hover:bg-surface-container" onClick={() => openEditModal(opp)} type="button"><span className="material-symbols-outlined text-[18px]" aria-hidden="true">edit</span>Edit</button>
                          <button role="menuitem" className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left font-body-sm text-body-sm text-on-surface hover:bg-surface-container" onClick={() => copyOpportunityLink(opp)} type="button"><span className="material-symbols-outlined text-[18px]" aria-hidden="true">link</span>Copy application link</button>
                          <button role="menuitem" className="flex w-full items-center gap-2.5 rounded-xl px-3 py-2 text-left font-body-sm text-body-sm text-error hover:bg-surface-container" onClick={() => { setOpenMenuId(null); setDeleteTarget(opp); }} type="button"><span className="material-symbols-outlined text-[18px]" aria-hidden="true">delete</span>Delete</button>
                        </div>
                      ) : null}
                    </div>
                  </div>
              ...
              <div className="flex items-center justify-between pt-space-sm border-t-0 bg-surface-container-low/50 -mx-space-lg -mb-space-lg p-space-md px-space-lg rounded-b-2xl">
                <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-on-surface-variant font-label-sm text-label-sm">
                  ...
                  {deadline ? (
                    <span className="font-semibold text-primary">Apply by {deadline}</span>
                  ) : startDate ? (
                ...
                <div className="flex items-center gap-2">
                  <button aria-label={`View details for ${title}`} className="rounded-full p-2.5 text-on-surface transition-colors hover:bg-secondary-container hover:text-on-secondary-fixed" onClick={() => setDetailsOpp(opp)} type="button"><span className="material-symbols-outlined text-[20px]">visibility</span></button>
                  <a
                    href={applyUrl}
                    target={applyUrl === '#' ? undefined : '_blank'}
                    rel={applyUrl === '#' ? undefined : 'noreferrer'}
